...
rm -f D:\project\wings-k8s-260703\dryrun_outputs\deepseek_v4_pd_project\prefill\progress.jsonl
...
find D:\project\wings-k8s-260703\dryrun_outputs\deepseek_v4_pd_project\prefill/log_analyzer -name '__pycache__' -type d -exec rm -rf {} + 2>/dev/null || true
cd D:\project\wings-k8s-260703\dryrun_outputs\deepseek_v4_pd_project\prefill && python3 -B -m log_analyzer.log_analyzer \
    --config "$ANALYZER_CONFIG" \
    --log-file /var/log/wings/engine.log \
    --progress-file D:\project\wings-k8s-260703\dryrun_outputs\deepseek_v4_pd_project\prefill\progress.jsonl &
LOG_ANALYZER_PID=$!
...
cleanup_analyzer() {
    ...
    if [ -n "${ENGINE_PID:-}" ]; then
        ...
        if [ "$exit_code" -ne 0 ]; then
            ...
            cat >> "D:\project\wings-k8s-260703\dryrun_outputs\deepseek_v4_pd_project\prefill\progress.jsonl" <<EARLY_FAIL_EOF
...
export HCCL_IF_IP=10.254.124.131
...
(
  ...
  for i in $(seq 0 0); do
    ...
    ASCEND_RT_VISIBLE_DEVICES=$CARDS vllm serve 'D:\project\wings-k8s-260703\dryrun_outputs\deepseek_v4_pd_project\fake_model' --trust-remote-code --max-model-len 1048576 --max-num-batched-tokens 8192 --gpu-memory-utilization 0.9 --api-server-count 1 --max-num-seqs 16 --enable-expert-parallel --quantization ascend --block-size 128 --safetensors-load-strategy prefetch --model-loader-extra-config '{"enable_multithread_load":"true","num_threads":128}' --additional-config '{"enable_cpu_binding":true,"enable_shared_expert_dp":true,"enable_dsa_cp":true}' --tokenizer-mode deepseek_v4 --tool-call-parser deepseek_v4 --enable-auto-tool-choice --host 10.254.124.131 --served-model-name Eco-Tech/DeepSeek-V4-Flash-w8a8-mtp --default-chat-template-kwargs '{"thinking":false}' --kv-transfer-config '{"kv_connector":"MooncakeHybridConnector","kv_role":"kv_producer","kv_port":"'"$KVPORT"'","kv_connector_extra_config":{"prefill":{"dp_size":2,"tp_size":4},"decode":{"dp_size":8,"tp_size":1}},"engine_id":"'"$PD_INDEX"'"}' --seed 1024 --no-enable-prefix-caching --reasoning-parser deepseek_v4 --no-disable-hybrid-kv-cache-manager --speculative-config '{"num_speculative_tokens":1,"method":"mtp","enforce_eager":true}' --enforce-eager --port $PORT --tensor-parallel-size 4 --data-parallel-size 2 --data-parallel-rank $RANK --data-parallel-size-local 1 --data-parallel-address 10.254.124.131 --data-parallel-rpc-port 12890 --data-parallel-external-lb &
    pids+=($!)
  done
  wait -n || true
...
echo "[Engine] Engine PID: $ENGINE_PID (retry mode)"
  ...
  if wait "$ENGINE_PID"; then
    ...
    echo "[Engine] Retry also failed, exit_code=$EXIT_CODE — unrecoverable"
      ...
      cat >> "D:\project\wings-k8s-260703\dryrun_outputs\deepseek_v4_pd_project\prefill\progress.jsonl" <<EOF
