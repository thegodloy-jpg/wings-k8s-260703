# -*- coding: utf-8 -*-
"""vLLM单机单GPU插件。

针对vLLM单机单GPU部署场景的日志分析插件。
"""

from __future__ import annotations

from typing import Dict, List

from ..log_analyzer import BaseLogPatternPlugin


class VLLMSingleGPUPlugin(BaseLogPatternPlugin):
    """vLLM单机单GPU插件。"""
    def get_error_patterns(self) -> List[Dict]:
        """返回错误匹配模式。

        每个 pattern 必须足够精确，避免在日志行中仅因提及错误
        类型名称即触发误判。对 Python 异常类型统一要求尾随冒号
        （如 ``RuntimeError:``），确保匹配的是实际异常行而非普通
        日志文本。
        """
        return [
            {
                "pattern": r"Out of memory",
                "error_type": "OOM"
            },
            {
                "pattern": r"RuntimeError:",
                "error_type": "RuntimeError"
            },
            {
                "pattern": r"ValueError:",
                "error_type": "ValueError"
            },
            {
                "pattern": r"AttributeError:",
                "error_type": "AttributeError"
            },
            {
                "pattern": r"(?:Import|ModuleNotFound)Error:",
                "error_type": "ImportError"
            },
            {
                "pattern": r"FileNotFoundError:",
                "error_type": "FileNotFoundError"
            },
            {
                "pattern": r"Application startup failed",
                "error_type": "StartupFailed"
            },
            {
                "pattern": r"Traceback \(most recent call last\):",
                "error_type": "Exception"
            }
        ]

    def get_stage_definitions(self) -> Dict[str, Dict]:
        """返回阶段定义和权重。"""
        return {
            "init": {
                "weight": 5,
                "name": "wings-control初始化"
            },
            "accel_enabling": {
                "weight": 15,
                "name": "加速特性注入"
            },
            "engine_booting": {
                "weight": 10,
                "name": "engine初始化"
            },
            "model_loading": {
                "weight": 30,
                "name": "模型加载"
            },
            "model_compiling": {
                "weight": 15,
                "name": "模型编译"
            },
            "cuda_graph_capturing": {
                "weight": 23,
                "name": "CUDA图捕获"
            },
            "server_checking": {
                "weight": 1,
                "name": "服务启动"
            },
            "ready": {
                "weight": 1,
                "name": "启动终态"
            }
        }

    def get_log_patterns(self) -> List[Dict]:
        """返回日志匹配模式。"""
        patterns = []
        patterns.extend(self._get_engine_booting_patterns())
        patterns.extend(self._get_model_loading_patterns())
        patterns.extend(self._get_model_compiling_patterns())
        patterns.extend(self._get_cuda_graph_capturing_patterns())
        patterns.extend(self._get_server_checking_patterns())
        patterns.extend(self._get_ready_patterns())
        return patterns

    def _get_engine_booting_patterns(self) -> List[Dict]:
        """获取engine_booting阶段的日志模式。"""
        return [
            {
                "pattern": r"non-default args:\s*(.+)",
                "phase_code": "engine_booting",
                "progress_calc": 22
            },
            {
                "pattern": r"Resolved architecture:\s*(.+)",
                "phase_code": "engine_booting",
                "progress_calc": 25
            },
            {
                "pattern": r"Initializing a V1 LLM engine",
                "phase_code": "engine_booting",
                "progress_calc": 28
            },
            {
                "pattern": r"world_size=\d+\s+rank=\d+\s+local_rank=\d+",
                "phase_code": "engine_booting",
                "progress_calc": 30
            }
        ]

    def _get_model_loading_patterns(self) -> List[Dict]:
        """获取model_loading阶段的日志模式。"""
        return [
            {
                "pattern": r"Starting to load model",
                "phase_code": "model_loading",
                "progress_calc": 32
            },
            {
                "pattern": r"Loading safetensors checkpoint shards:\s*(\d+)%",
                "phase_code": "model_loading",
                "progress_calc": lambda m, curr: int(36 + (int(m.group(1)) * 0.24))
            },
            {
                "pattern": r"Model loading took\s+(\d+\.?\d*)\s+seconds",
                "phase_code": "model_loading",
                "progress_calc": 60
            }
        ]

    def _get_model_compiling_patterns(self) -> List[Dict]:
        """获取model_compiling阶段的日志模式。"""
        return [
            {
                "pattern": r"Using cache directory.*torch_compile_cache",
                "phase_code": "model_compiling",
                "progress_calc": 62
            },
            {
                "pattern": r"Dynamo bytecode transform time:\s*(\d+\.?\d*)",
                "phase_code": "model_compiling",
                "progress_calc": 66
            },
            {
                "pattern": r"Cache the graph of compile range",
                "phase_code": "model_compiling",
                "progress_calc": 68
            },
            {
                "pattern": r"Compiling a graph for compile range",
                "phase_code": "model_compiling",
                "progress_calc": 70
            },
            {
                "pattern": r"torch\.compile takes\s+(\d+\.?\d*)\s+s?\s+in total",
                "phase_code": "model_compiling",
                "progress_calc": 73
            },
            {
                "pattern": r"Available KV cache memory",
                "phase_code": "model_compiling",
                "progress_calc": 75
            }
        ]

    def _get_cuda_graph_capturing_patterns(self) -> List[Dict]:
        """获取cuda_graph_capturing阶段的日志模式。"""
        return [
            {
                "pattern": r"Capturing CUDA graphs \(mixed prefill-decode, PIECEWISE\):\s*(\d+)%",
                "phase_code": "cuda_graph_capturing",
                "progress_calc": lambda m, curr: int(76 + (int(m.group(1)) * 0.12))
            },
            {
                "pattern": r"Capturing CUDA graphs \(decode, FULL\):\s*(\d+)%",
                "phase_code": "cuda_graph_capturing",
                "progress_calc": lambda m, curr: int(88 + (int(m.group(1)) * 0.10))
            },
            {
                "pattern": r"Graph capturing finished in\s+(\d+\.?\d*)\s+secs",
                "phase_code": "cuda_graph_capturing",
                "progress_calc": 98
            }
        ]

    def _get_server_checking_patterns(self) -> List[Dict]:
        """获取server_checking阶段的日志模式。"""
        return [
            {
                "pattern": r"init engine.*seconds",
                "phase_code": "server_checking",
                "progress_calc": 99
            },
            {
                "pattern": r"Application startup complete",
                "phase_code": "server_checking",
                "progress_calc": 99
            },
            {
                "pattern": r"health_checking",
                "phase_code": "server_checking",
                "progress_calc": 99
            }
        ]

    def _get_ready_patterns(self) -> List[Dict]:
        """获取ready阶段的日志模式。"""
        return [
            {
                "pattern": r"health=200",
                "phase_code": "ready",
                "progress_calc": 100
            },
            {
                "pattern": r"GET /health HTTP/1\.1\" 200 OK",
                "phase_code": "ready",
                "progress_calc": 100
            },
            {
                "pattern": r"Starting vLLM server on",
                "phase_code": "ready",
                "progress_calc": 100
            },
            {
                "pattern": r"Available routes are",
                "phase_code": "ready",
                "progress_calc": 100
            }
        ]
