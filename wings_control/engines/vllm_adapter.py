...
def _resolve_native_backend_offload_gb(params: Dict[str, Any], engine: str) -> int:
    ...
    ``LMCACHE_MAX_LOCAL_CPU_SIZE`` 兼容入口。
    """
...
def _native_backend_auto_requested(params: Dict[str, Any], engine: str) -> bool:
    ...
    native backend 的用户容量入口统一为 ``KV_MEM_OFFLOAD_SIZE``，这样 CLI、
    variant 和 `/v1/startup/accel` 中的 floor_disabled 状态保持同源。
    """
...
_OFFLOAD_MEMCACHE_AUTO_FLOOR_VARIANT = (
    f"{memcache_hybrid.MEMCACHE_OFFLOAD_VARIANT}+auto+floor_disabled"
)
_OFFLOAD_MEMCACHE_AUTO_UNAVAILABLE_VARIANT = (
    f"{memcache_hybrid.MEMCACHE_OFFLOAD_VARIANT}+{_OFFLOAD_AUTO_UNAVAILABLE_MODIFIER}"
)
_OFFLOAD_LMCACHE_AUTO_UNAVAILABLE_VARIANT = (
    f"lmcache_cpu+{_OFFLOAD_AUTO_UNAVAILABLE_MODIFIER}"
)
_OFFLOAD_INACTIVE_VARIANTS = {
    "disabled",
    _OFFLOAD_AUTO_FLOOR_VARIANT,
    _OFFLOAD_LMCACHE_AUTO_UNAVAILABLE_VARIANT,
    f"{_OFFLOAD_NATIVE_BACKEND_VARIANT}+auto+floor_disabled",
    f"{_OFFLOAD_NATIVE_BACKEND_VARIANT}+{_OFFLOAD_AUTO_UNAVAILABLE_MODIFIER}",
    _OFFLOAD_MEMCACHE_AUTO_FLOOR_VARIANT,
    _OFFLOAD_MEMCACHE_AUTO_UNAVAILABLE_VARIANT,
}
...
def _resolve_lmcache_cpu_env(params: Optional[Dict[str, Any]]) -> Tuple[str, str]:
    ...
    熔断(<=0)清空 CPU 池；非 auto 仅接受页面/上层下发的正整数
    ``KV_MEM_OFFLOAD_SIZE``。未填、非法或非正值直接丢弃，不再使用
    ``LMCACHE_MAX_LOCAL_CPU_SIZE`` 或模型级默认容量作为输入。需求一 §3.0。
    """
    ...
    if not max_cpu_size:
        logger.warning("[KVCache Offload] KV_MEM_OFFLOAD_SIZE is empty; skip CPU offload pool.")
        return "", ""

    if max_cpu_size.lower() != "auto":
        try:
            total_gb = int(max_cpu_size)
        except ValueError:
            logger.warning("[KVCache Offload] Invalid KV_MEM_OFFLOAD_SIZE=%r; skip CPU offload pool.", max_cpu_size)
            return "", ""
        if total_gb <= 0:
            logger.warning("[KVCache Offload] non-positive KV_MEM_OFFLOAD_SIZE=%s; skip CPU offload pool.", total_gb)
            return "", ""
        n_card = _safe_int(params.get("device_count")) if params else 1
        n_card = n_card or 1
        per_card = max(1, total_gb // n_card)
        logger.info(
    ...
    if auto_total is None:
        logger.warning(
            "[KVCache Offload] KV_MEM_OFFLOAD_SIZE=auto but auto capacity is unavailable; "
            "skip CPU offload pool."
        )
        return "", ""
    if auto_total <= 0:
...
def _resolve_auto_kv_mem_offload_drop_modifier(params: Optional[Dict[str, Any]]) -> str:
    """返回 auto 内存卸载被丢弃时写入状态 JSON 的诊断修饰。

    这里仅描述页面状态，不改变 engine 命令：floor_disabled 表示 auto 已算出但
    低于下限；unavailable 表示 auto 缺少可计算输入（例如 AVAILABLE_POD_MEM_SIZE）。
    """
    if os.getenv("ENABLE_KV_MEM_OFFLOAD", "false").strip().lower() != "true":
        return ""
    if os.getenv("KV_MEM_OFFLOAD_SIZE", "").strip().lower() != "auto":
        return ""
    auto_total = resolve_offload_cpu_capacity_gb(params or {})
    if auto_total == 0:
        return "floor_disabled"
    if auto_total is None:
        return "unavailable"
    return ""


def offload_variant_has_active_backend(variant: Optional[str]) -> bool:
    """判断 offload variant 是否代表会进入 engine 启动命令的真实后端。"""
    if not variant:
        return False
    return variant not in _OFFLOAD_INACTIVE_VARIANTS


def resolve_kv_offload_effective_state(
    params: Optional[Dict[str, Any]],
    engine: str,
) -> Tuple[bool, Optional[str]]:
    """统一解析状态 JSON 和启动字段生成都要消费的 KV offload 最终状态。

    请求开关只表示用户/白名单尝试启用；真正能否下发 connector/env/CLI，必须
    以后端 variant 是否活跃为准，避免页面已回显 false 时启动命令仍残留卸载字段。
    """
    if not _is_kv_offload_requested(params):
        return False, None
    variant = resolve_offload_variant(params, engine)
...
def _resolve_offload_backend(params: Optional[Dict[str, Any]], engine: str = "") -> Tuple[str, str]:
    ...
    raw_mem_size = os.getenv("KV_MEM_OFFLOAD_SIZE", "").strip()
    auto_total = resolve_offload_cpu_capacity_gb(params)
    if auto_total is not None:
        ...
        cpu_mode = ""
        if (
            os.getenv("ENABLE_KV_MEM_OFFLOAD", "false").strip().lower() == "true"
            and raw_mem_size
            and raw_mem_size.lower() != "auto"
        ):
            try:
                has_cpu = int(raw_mem_size) > 0
            except ValueError:
                has_cpu = False
...
def _build_cache_env_commands(engine: str, params: Optional[Dict[str, Any]] = None) -> List[str]:
    ...
    if (
        ...
        and not get_qat_env()
        and not get_cold_start_env()
    ):
        logger.info(
            "[KVCache Offload] memory offload requested but KV_MEM_OFFLOAD_SIZE is "
            "empty/invalid/unavailable; skipping LMCache engine-side env exports."
        )
        return env_commands
...
def resolve_effective_kv_mem_offload_size(
    ...
    if not resolved_variant or resolved_variant == "disabled":
        return None
    if "floor_disabled" in resolved_variant or "auto+unavailable" in resolved_variant:
        return 0
...
def is_deepseek_v4_pro_0813_w4a8_910c_dual_node_scope(
    params: Dict[str, Any],
    model_info: Optional[ModelIdentifier] = None,
) -> bool:
    ...
    identity_matches = any(
        ...
        .lower()
        == target_name
        for value in (params.get("model_name"), params.get("model_path"))
    )
    return identity_matches and _deepseek_v4_arch_matches(params, model_info) is not False
...
def _prepare_engine_config(params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    3. 最后处理旧版 ``task`` 字段、强制错误栈日志和 DP 拓扑回写。
...
def resolve_offload_cpu_capacity_gb(
    ...
    """兼容旧调用名：auto 容量公式统一复用 ``utils.env_utils`` 实现。"""
    return _resolve_offload_cpu_capacity_gb(params, size_env_name=size_env_name)
...
def _resolve_v4_flash_offload_gb(params: Dict[str, Any]) -> int:
    ...
    取值规则（整节点口径，两路径同源同值）：
      ...
      * ``KV_MEM_OFFLOAD_SIZE`` 未设/非法 → 直接丢弃，不再使用场景默认值；
...
def _resolve_minimax_m27_lmcache_max_cpu_size(params: Dict[str, Any]) -> Optional[str]:
    ...
    raw_size = os.getenv("KV_MEM_OFFLOAD_SIZE", "").strip()
    if not raw_size:
        logger.warning("[MiniMax-M2.7] KV_MEM_OFFLOAD_SIZE is empty; skip LMCACHE_MAX_LOCAL_CPU_SIZE.")
        return None
    ...
    if total <= 0:
        logger.warning(
            "[MiniMax-M2.7] non-positive KV_MEM_OFFLOAD_SIZE=%s; skip LMCACHE_MAX_LOCAL_CPU_SIZE.",
            total,
        )
        return None
...
def _build_minimax_m27_rtx_pro_5000_env_commands(params: Dict[str, Any]) -> List[str]:
    ...
    if not (mem_offload or disk_offload):
        return cmds
    offload_active, offload_variant = resolve_kv_offload_effective_state(params, "vllm")
    if not offload_active:
        # MiniMax 专属 env 不经过通用 _build_cache_env_commands，需在这里复用最终
        # offload 状态，避免 auto 丢弃或无有效容量时仍残留 LMCACHE_* 启动字段。
        logger.info(
            "[MiniMax-M2.7] offload variant=%s is inactive; skip LMCache runtime env.",
            offload_variant,
        )
        return cmds
