...
def _set_common_params(params, engine_cmd_parameter, config_path):
    ...
    默认值补充；其他参数仍按旧逻辑补充。max_num_seqs / max_num_batched_tokens
    保留 launcher 兼容默认 32 / 4096，用于页面未下发时继续拼接到引擎命令。
    """
...
def _set_kv_cache_config(params, ctx, model_info=None):
    ...
    if lmcache_offload and offload_backend == "simple_cpu":
        if pd_role:
            ...
            # 已验证的 H20 调优场景只注入 SimpleCPU connector；提前返回以隔离其它卸载后端。
            effective_ctx = dict(ctx)
            effective_ctx["engine_config"] = params
            connector_config = resolve_simple_cpu_offload_config(
                effective_ctx,
                ctx.get("engine", ""),
            )
