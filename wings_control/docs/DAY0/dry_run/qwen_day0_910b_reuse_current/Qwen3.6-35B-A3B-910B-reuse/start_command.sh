...
ANALYZER_CONFIG='{"engine": "vllm_ascend", "deployment_mode": "single", "hardware": "ascend", "nnodes": 1, "node_rank": 0, "distributed_backend": "mp", "tensor_parallel_size": 2, "model_name": "Qwen/Qwen3.6-35B-A3B", "model_path": "D:\\project\\wings-k8s-260703\\wings_control\\docs\\DAY0\\dry_run\\qwen_day0_910b_reuse_current\\_model_configs\\Qwen3.6-35B-A3B-910B-reuse", "backend_port": 7799}'
...
if [ -f "/accel-volume/install.py" ]; then
    ...
    if [ $SPEC_RC -ne 0 ]; then
        ...
        python3 -c "import json, os; p='D:\project\wings-k8s-260703\wings_control\docs\DAY0\dry_run\qwen_day0_910b_reuse_current\advanced_features.json'; d=json.load(open(p)) if os.path.exists(p) else {'engine':'','features':{}}; d.setdefault('features',{})['speculative_decode']=False; f=open(p+'.tmp','w'); json.dump(d,f,indent=4); f.close(); os.replace(p+'.tmp',p)"
    else
...
export WINGS_MEMCACHE_META_SERVICE_URL="${WINGS_MEMCACHE_META_SERVICE_URL:-tcp://127.0.0.1:50051}"
echo "[wings-env] export WINGS_MEMCACHE_META_SERVICE_URL=${WINGS_MEMCACHE_META_SERVICE_URL:-}"
export WINGS_MEMCACHE_CONFIG_STORE_URL="${WINGS_MEMCACHE_CONFIG_STORE_URL:-tcp://127.0.0.1:50061}"
...
echo '[wings-cmd] >>> exec python3 -m vllm.entrypoints.openai.api_server --trust-remote-code --max-model-len 131072 --seed 1024 --tensor-parallel-size 2 --data-parallel-size 1 --enable-expert-parallel --max-num-seqs 32 --max-num-batched-tokens 8192 --gpu-memory-utilization 0.9 --served-model-name qwen36 --async-scheduling --compilation-config '"'"'{"cudagraph_mode":"FULL_DECODE_ONLY"}'"'"' --additional-config '"'"'{"enable_cpu_binding":true,"multistream_overlap_shared_expert":true}'"'"' --tool-call-parser qwen3_coder --host 127.0.0.1 --port 7799 --model '"'"'D:\project\wings-k8s-260703\wings_control\docs\DAY0\dry_run\qwen_day0_910b_reuse_current\_model_configs\Qwen3.6-35B-A3B-910B-reuse'"'"' --enable-auto-tool-choice --default-chat-template-kwargs '"'"'{"enable_thinking":false}'"'"' --kv-transfer-config '"'"'{"kv_connector":"AscendStoreConnector...<truncated>'
python3 -m vllm.entrypoints.openai.api_server --trust-remote-code --max-model-len 131072 --seed 1024 --tensor-parallel-size 2 --data-parallel-size 1 --enable-expert-parallel --max-num-seqs 32 --max-num-batched-tokens 8192 --gpu-memory-utilization 0.9 --served-model-name qwen36 --async-scheduling --compilation-config '{"cudagraph_mode":"FULL_DECODE_ONLY"}' --additional-config '{"enable_cpu_binding":true,"multistream_overlap_shared_expert":true}' --tool-call-parser qwen3_coder --host 127.0.0.1 --port 7799 --model 'D:\project\wings-k8s-260703\wings_control\docs\DAY0\dry_run\qwen_day0_910b_reuse_current\_model_configs\Qwen3.6-35B-A3B-910B-reuse' --enable-auto-tool-choice --default-chat-template-kwargs '{"enable_thinking":false}' --kv-transfer-config '{"kv_connector":"AscendStoreConnector","kv_role":"kv_both","kv_load_failure_policy":"recompute","kv_connector_extra_config":{"lookup_rpc_port":"0","backend":"memcache"}}' --speculative-config '{"method": "qwen3_5_mtp", "num_speculative_tokens": 3, "enforce_eager": true}' &
ENGINE_PID=$!
...
if wait "$ENGINE_PID"; then
  ...
  cat > "D:\project\wings-k8s-260703\wings_control\docs\DAY0\dry_run\qwen_day0_910b_reuse_current\advanced_features.json" <<'FEATURES_EOF'
{
    "engine": "vllm_ascend",
    "features": {
