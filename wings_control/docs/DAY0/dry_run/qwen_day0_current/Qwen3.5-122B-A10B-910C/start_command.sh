...
ANALYZER_CONFIG='{"engine": "vllm_ascend", "deployment_mode": "single", "hardware": "ascend", "nnodes": 1, "node_rank": 0, "distributed_backend": "mp", "tensor_parallel_size": 8, "model_name": "Qwen/Qwen3.5-122B-A10B", "model_path": "D:/project/wings-k8s-260703/wings_control/docs/DAY0/dry_run/qwen_day0_current/_model_configs/Qwen3.5-122B-A10B-910C", "backend_port": 6901}'
...
echo "[wings-env] export HCCL_OP_EXPANSION_MODE=${HCCL_OP_EXPANSION_MODE:-}"
echo '[wings-cmd] >>> exec python3 -m vllm.entrypoints.openai.api_server --trust-remote-code --max-model-len 131072 --seed 1024 --tensor-parallel-size 8 --data-parallel-size 1 --enable-expert-parallel --max-num-seqs 8 --max-num-batched-tokens 16384 --gpu-memory-utilization 0.9 --served-model-name qwen35 --language-model-only --async-scheduling --compilation-config '"'"'{"cudagraph_mode":"FULL_DECODE_ONLY"}'"'"' --additional-config '"'"'{"enable_cpu_binding":false,"ascend_compilation_config":{"enable_npugraph_ex":true,"enable_static_kernel":false},"multistream_overlap_shared_expert":false}'"'"' --tool-call-parser qwen3_coder --host 127.0.0.1 --port 6901 --model D:/project/wings-k8s-260703/wings_control/docs/DAY0/dry_run/qwen_day0_current/_model_configs/Qwen3.5-122B-A10B-910C --enable-auto-tool-choice --default-chat-template-kwa...<truncated>'
python3 -m vllm.entrypoints.openai.api_server --trust-remote-code --max-model-len 131072 --seed 1024 --tensor-parallel-size 8 --data-parallel-size 1 --enable-expert-parallel --max-num-seqs 8 --max-num-batched-tokens 16384 --gpu-memory-utilization 0.9 --served-model-name qwen35 --language-model-only --async-scheduling --compilation-config '{"cudagraph_mode":"FULL_DECODE_ONLY"}' --additional-config '{"enable_cpu_binding":false,"ascend_compilation_config":{"enable_npugraph_ex":true,"enable_static_kernel":false},"multistream_overlap_shared_expert":false}' --tool-call-parser qwen3_coder --host 127.0.0.1 --port 6901 --model D:/project/wings-k8s-260703/wings_control/docs/DAY0/dry_run/qwen_day0_current/_model_configs/Qwen3.5-122B-A10B-910C --enable-auto-tool-choice --default-chat-template-kwargs '{"enable_thinking":false}' &
ENGINE_PID=$!
