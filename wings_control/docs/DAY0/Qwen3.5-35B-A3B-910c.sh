#!/bin/bash
# Qwen3.5-35B-A3B feature run on 910C: EP + MTP, no MemCache offload.
set +e

MODEL_DIR=${MODEL_DIR:-/var/aispace/model/ai-storage/ai-prod/platform/Qwen3.5-35B-A3B}
SERVED=${SERVED:-qwen35}
PORT=${PORT:-7897}
GPU_MEM_UTIL=${GPU_MEM_UTIL:-0.90}
LOG_DIR=${LOG_DIR:-/var/aispace/jsh/work/day0-jsh/logs5}
mkdir -p "$LOG_DIR"

export ASCEND_RT_VISIBLE_DEVICES=${ASCEND_RT_VISIBLE_DEVICES:-2,3}
...
vllm serve "$MODEL_DIR" \
    ...
    --max-num-seqs 32 --max-model-len 161072 --max-num-batched-tokens 8192 \
    ...
    > "$LOG_DIR/qwen35_35b_a3b-$(date +%Y%m%d_%H%M%S).log" 2>&1
