#!/bin/bash
# Qwen3.5-27B feature run on 910C: dense bf16 + MTP, optional MemCache offload.
set +e
...
export ASCEND_RT_VISIBLE_DEVICES=${ASCEND_RT_VISIBLE_DEVICES:-10,11}
...
LOG_FILE="$LOG_DIR/qwen35_27b_910c-$(date +%Y%m%d_%H%M%S).log"

KV_ARGS=()
if [[ "$ENABLE_MEMCACHE" == "1" || "$ENABLE_MEMCACHE" == "true" ]]; then
  export MMC_LOCAL_CONFIG_PATH=${MMC_LOCAL_CONFIG_PATH:-/var/aispace/jsh/work/day0-jsh/confs/mmc_local1.conf}
...
vllm serve "$MODEL_DIR" \
    ...
    --max-num-seqs 8 --max-model-len 131072 --max-num-batched-tokens 16384 \
