#!/bin/bash
# Qwen3.5-122B-A10B feature run on 910C: bf16 MoE + EP + MTP, no MemCache offload.
set +e

cd "$(dirname "$0")"
MODEL_DIR=${MODEL_DIR:-/var/aispace/model/ai-storage/ai-prod/platform/Qwen3.5-122B-A10B}
...
export ASCEND_RT_VISIBLE_DEVICES=${ASCEND_RT_VISIBLE_DEVICES:-6,7,10,11,12,13,14,15}
...
vllm serve "$MODEL_DIR" \
    ...
    --async-scheduling \
    > "$LOG_DIR/122b_mtp-$(date +%Y%m%d_%H%M%S).log" 2>&1
