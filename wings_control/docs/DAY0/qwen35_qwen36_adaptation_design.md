...
本次实现额外补充 4 个 910B 专属默认配置 key，便于后续在 `ascend_default.json` 中独立演进；它们暂时完全复用对应 910C 的优化参数，不改变上方 11 个 Day0 标准场景的事实源。

| 扩展场景 | 默认参数来源 | 特性策略 | 说明 |
| --- | --- | --- | --- |
| Qwen3.5-35B-A3B-910B | 复用 Qwen3.5-35B-A3B-910C | `qwen3_5_mtp`, tokens=1, MemCache | 独立默认配置 key；命中 910B spec/offload 白名单，offload backend 为 memcache |
...
| Qwen3.6-27B-w8a8-910B | row7 / 910B 优化 | done | Eco-Tech/Qwen3.6-27B-w8a8 | vllm_ascend | 910B | INT8 / W8A8 | 4 | 1 | 7899 | qwen3_5_mtp | 3 | MTP+MemCache | 是 | 50071/50081 | 否 | quantization=ascend | 32 | 131072 | 8192 | 0.9 | 1024 | qwen36 | 是 | qwen3_coder | 否 |
...
- `auto` 先按 `container_gb - (7 * TP * DP + 3) - container_gb * 0.10` 计算节点总容量，沿用通用 LMCache CPU 池的 `20G` floor，再按 `device_count` 向下均分；
...
- `smart_feature_whitelist.json` 中 Qwen Day0 `offload` 覆盖 5 个 910C 场景，并单独补齐 Qwen3.6-27B-w8a8、Qwen3.6-35B-A3B-w8a8、Qwen3.5-35B-A3B 三个 910B MemCache 场景。
...
- 未列入 910B MemCache 白名单的 Qwen3.5 / Qwen3.6 910B 场景请求卸载后，有效 offload 被白名单关闭，最终命令没有 MemCache / LMCache 片段，但 MTP 保持有效；三个已补齐的 910B MemCache 场景应输出 AscendStoreConnector。
