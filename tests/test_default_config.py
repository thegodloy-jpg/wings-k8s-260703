...
def test_vllm_capacity_defaults_do_not_reach_vllm_commands_when_not_explicit(monkeypatch):
    monkeypatch.setattr(sys, "argv", ["pytest"])
    _clear_removed_param_env(monkeypatch)
    engine_cmd_parameter = {
        "max_num_seqs": 32,
    ...
    for engine in ("vllm", "vllm_ascend"):
        ...
        assert "max_num_seqs" not in engine_config
        assert "max_num_batched_tokens" not in engine_config
        assert "--max-num-seqs 32" not in command
        assert "--max-num-batched-tokens 4096" not in command
